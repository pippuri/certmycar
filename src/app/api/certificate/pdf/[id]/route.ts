...
export async function POST(
  ...
  try {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <p><strong>Health Score:</strong> ${Math.round(100 - certificate.battery_health_data.degradation_percentage)}%</p>
            <p><strong>Certificate ID:</strong> ${certificate.certificate_id}</p>
            <p><strong>Generated:</strong> ${new Date(certificate.created_at).toLocaleDateString()}</p>
          </div>
          
          <p>You can also verify this certificate online at:</p>
          <p><a href="${process.env.NEXT_PUBLIC_SITE_URL || 'https://batterycert.com'}/${id}?vin=${certificate.tesla_vin}" style="color: #2563eb;">View Certificate Online</a></p>
    ...
    try {
      ...
      const supabase = await createServerSupabaseClient();
      const { error: emailLogError } = await supabase.from('certificate_emails').insert({
        certificate_id: id,
        email_address: email,
