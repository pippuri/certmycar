...
const testCertificates = [
  {
    ...
    battery_health_data: {
      ...
      methodology:
        "Tesla Ideal Range Analysis (NCM/LG Chem cells) - Vehicle from Shanghai, China (Giga Shanghai) (LG Generation 2 - 75kWh usable)",
  ...
  },
  {
    ...
    battery_health_data: {
      ...
      confidence_level: "high",
      methodology:
        "Tesla Ideal Range Analysis (LFP/CATL chemistry) - Vehicle from Shanghai, China (Giga Shanghai)",
  ...
  },
  {
    ...
    battery_health_data: {
      ...
      confidence_level: "high",
      methodology:
        "Tesla Ideal Range Analysis (NCM/LG Chem cells) - Vehicle from Berlin, Germany (Giga Berlin)",
      battery_chemistry: "NCM",
      battery_supplier: "LG",
...
export async function POST() {
  try {
    ...
    const { error: deleteError } = await supabase
      ...
      .in(
        "certificate_id",
        testCertificates.map((c) => c.certificate_id)
      );

    if (deleteError) {
      console.log(
        "No existing certificates to delete or delete failed:",
        deleteError.message
      );
    } else {
      ...
      return NextResponse.json(
        {
          ...
          error: error.message,
          suggestion:
            "The certificates table may not exist. Please check the DATABASE_SETUP.md file for SQL to create it.",
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      {
        ...
        setup_required:
          "The database tables may not exist. Check DATABASE_SETUP.md for setup instructions.",
      },
      { status: 500 }
    );
  }
...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      .from("certificates")
      .select(
        "certificate_id, vehicle_name, vehicle_model, tesla_vin, battery_health_data, created_at"
      )
      ...
      return NextResponse.json(
        {
          ...
          error: error.message,
          setup_required:
            "The certificates table may not exist. Check DATABASE_SETUP.md for setup instructions.",
        },
        { status: 500 }
      );
    }
