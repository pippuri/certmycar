...
export async function GET() {
  ...
  try {
    ...
    if (!TESLA_CLIENT_ID || !TESLA_CLIENT_SECRET) {
      return NextResponse.json(
        { 
          ...
          message: "TESLA_CLIENT_ID and TESLA_CLIENT_SECRET must be set in environment variables"
        },
        { status: 400, headers }
      );
    }
    ...
    const registerResponse = await fetch(`${TESLA_API_BASE}/api/1/partner_accounts`, {
      method: 'POST',
      headers: {
